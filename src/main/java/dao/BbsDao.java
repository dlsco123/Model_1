...
//				+ "	where rnum >= ? && rnum <=? ";
	...
	public BbsDto getBbs(int seq) {
		...
		try {
			conn = DBConnection.getConnection();
			System.out.println("1/3 getbbs");
			
			psmt = conn.prepareStatement(sql);
			psmt.setInt(1, seq);
			System.out.println("2/3 getbbs");
			
			rs = psmt.executeQuery();
			System.out.println("3/3 getbbs");
			if (rs.next()) {
	...
	public boolean answer(int seq, BbsDto dto) {
		...
		String sql1 = " update bbs "
				...
				+ "	&& step>(select step from (select step from bbs b where seq=?) B )";
		...
		try {
			conn = DBConnection.getConnection();
			// TODO (특수)commit 비활성화		commit(완료선언) <=> rollback
